...
namespace ergometer {
  ...
  export interface RowingAdditionalStrokeDataEvent
    extends pubSub.ISubscription {
  ...
  export interface RowingAdditionalSplitIntervalDataEvent
    extends pubSub.ISubscription {
  ...
  export interface AdditionalWorkoutSummaryDataEvent
    extends pubSub.ISubscription {
    (data: AdditionalWorkoutSummaryData): void;
  }
  export interface AdditionalWorkoutSummaryData2Event
    extends pubSub.ISubscription {
  ...
  export class PerformanceMonitorBle extends PerformanceMonitorBase {
    ...
    protected enableMultiplexNotification(): Promise<void> {
      ...
      if (this._multiplexSubscribeCount == 0)
        result = this.enableNotification(
          ble.PMROWING_SERVICE,
    ...
    protected disableMultiPlexNotification(): Promise<void> {
      ...
      this._multiplexSubscribeCount--;
      if (this._multiplexSubscribeCount == 0)
        result = this.disableNotification(
          ble.PMROWING_SERVICE,
          ble.MULTIPLEXED_INFO_CHARACTERISIC
    ...
    protected enableNotification(
      ...
      return this.driver.enableNotification(
        serviceUIID,
        characteristicUUID,
        receive
      );
    ...
    protected onPowerCurveRowingGeneralStatus(
      data: ergometer.RowingGeneralStatus
    ) {
    ...
    protected initDriver() {
      if (bleCentral.available())
        ...
        this._driver = new ble.DriverWebBlueTooth(
          this,
          [ble.PMDEVICE],
          [
    ...
    addDevice(device: DeviceInfo) {
      ...
      this._devices.sort((device1, device2: DeviceInfo): number => {
     ...
     */
    public startScan(
      deviceFound: (device: DeviceInfo) => boolean,
    ...
    protected handleRowingGeneralStatus(data: DataView) {
      var parsed: RowingGeneralStatus = {
        ...
        workoutType: data.getUint8(
          ble.PM_Rowing_Status_BLE_Payload.WORKOUT_TYPE
        ),
        ...
        rowingState: data.getUint8(
          ble.PM_Rowing_Status_BLE_Payload.ROWING_STATE
        ),
        strokeState: data.getUint8(
          ble.PM_Rowing_Status_BLE_Payload.STROKE_STATE
        ),
    ...
    protected handleRowingAdditionalStatus2(data: DataView) {
      ...
      if (
        data.byteLength == ble.PM_Extra_Status2_BLE_Payload.BLE_PAYLOAD_SIZE
      ) {
        parsed = {
          elapsedTime:
            utils.getUint24(
          ...
          lastSplitTime:
            data.getUint16(
              ble.PM_Extra_Status2_BLE_Payload.LAST_SPLIT_TIME_LO
        ...
        parsed = {
          ...
          strokeRecoveryTime:
            (data.getUint8(
              ...
              data.getUint8(
                ble.PM_Stroke_Data_BLE_Payload.STROKE_RECOVERY_TIME_HI
              ) *
                256) *
            10, //ms
          strokeDistance:
            (data.getUint8(
              ...
              data.getUint8(
                ble.PM_Mux_Stroke_Data_BLE_Payload.STROKE_DISTANCE_HI
              ) *
                256) /
            100, //meter
          peakDriveForce:
          ...
          workPerStroke:
            ...
            10, //jouls
          strokeCount:
            data.getUint8(ble.PM_Stroke_Data_BLE_Payload.STROKE_COUNT_LO) +
            data.getUint8(ble.PM_Stroke_Data_BLE_Payload.STROKE_COUNT_HI) * 256, //PM bug: LSB and MSB are swapped
        };
      } else {
        parsed = {
          elapsedTime:
          ...
          driveLength:
            data.getUint8(ble.PM_Mux_Stroke_Data_BLE_Payload.DRIVE_LENGTH) /
            100, //meters
          driveTime:
          ...
          workPerStroke: null,
          strokeCount:
            data.getUint8(ble.PM_Mux_Stroke_Data_BLE_Payload.STROKE_COUNT_LO) +
            data.getUint8(ble.PM_Mux_Stroke_Data_BLE_Payload.STROKE_COUNT_HI) *
    ...
    protected handleRowingAdditionalStrokeData(data: DataView) {
      var parsed: RowingAdditionalStrokeData = {
        ...
        strokePower:
          data.getUint8(ble.PM_Extra_Stroke_Data_BLE_Payload.STROKE_POWER_LO) +
          data.getUint8(ble.PM_Extra_Stroke_Data_BLE_Payload.STROKE_POWER_HI) *
            256, //watts
        strokeCalories: data.getUint16(
          ble.PM_Extra_Stroke_Data_BLE_Payload.STROKE_CALORIES_LO
        ), //cal/hr
        strokeCount:
          data.getUint8(ble.PM_Extra_Stroke_Data_BLE_Payload.STROKE_COUNT_LO) +
          data.getUint8(ble.PM_Extra_Stroke_Data_BLE_Payload.STROKE_COUNT_HI) *
            256, //PM bug: LSB and MSB are swapped
        projectedWorkTime:
          utils.getUint24(
            data,
      ...
      if (
        JSON.stringify(this.rowingAdditionalStrokeData) !==
    ...
    protected handleRowingSplitIntervalData(data: DataView) {
      var parsed: RowingSplitIntervalData = {
        ...
        intervalType: data.getUint8(
          ble.PM_Split_Interval_Data_BLE_Payload.TYPE
        ),
    ...
    protected handleRowingAdditionalSplitIntervalData(data: DataView) {
      var parsed: RowingAdditionalSplitIntervalData = {
        ...
        intervalSpeed:
          data.getUint16(
            ble.PM_Extra_Split_Interval_Data_BLE_Payload.SPEED_LO
          ) / 1000, //m/s
    ...
    protected handleHeartRateBeltInformation(data: DataView) {
      var parsed: HeartRateBeltInformation = {
        ...
        beltId: data.getUint32(
          ble.PM_Heart_Rate_Belt_Info_BLE_Payload.BELT_ID_LO
        ),
      };
      ...
      this.handleCSafeNotifications()
        ...
        .then(() => {
          ...
          this.changeConnectionState(
            MonitorConnectionState.readyForCommunication
          );
        })
    ...
    protected driver_write(data: ArrayBufferView): Promise<void> {
      return this.driver.writeCharacteristic(
        ble.PMCONTROL_SERVICE,
