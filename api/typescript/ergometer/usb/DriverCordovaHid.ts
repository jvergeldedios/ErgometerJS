    ...
    public sendData(data: ArrayBuffer): Promise<void> {
      if (data.byteLength > USB_CSAVE_SIZE)
        return Promise.reject(
          `Trying to send to much data, the buffer must be smaller or equal to ${USB_CSAVE_SIZE} and is ${data.byteLength}`
        );
      return new Promise((resolve, reject) => {
        try {
            ...
            .then((data: ArrayBuffer) => {
              ...
              setTimeout(() => {
                if (data && data.byteLength >= usb.WRITE_BUF_SIZE) {
                  ...
                  if (
                    endByte >= 1 &&
                    inputData.getUint8(endByte) == csafe.defs.FRAME_END_BYTE
                  ) {
          ...
          cordova.plugins.UsbHid.enumerateDevices()
            .then((devices) => {
              ...
              devices.forEach((device) => {
                ...
                if (
                  this._config.vendorIds.indexOf(parseInt(device.vendorId)) !==
                  -1
                ) {
