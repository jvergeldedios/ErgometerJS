...
namespace ergometer {
  ...
  export class WaitResponseBuffer implements ergometer.csafe.IResponseBuffer {
    ...
    removeRemainingCommands() {
      this._commands.forEach((command) => {
        if (this._monitor.logLevel >= LogLevel.error)
          this._monitor.handleError(
            `command removed without result command=${command.command} detail= ${command.detailCommand}`
          );
    ...
    private timeOut() {
      ...
      this.remove();
      if (this._reject) this._reject("Time out buffer");
      if (this._monitor.logLevel >= LogLevel.error)
        this._monitor.handleError("buffer time out");
    }
    constructor(
      monitor: PerformanceMonitorBase,
      resolve: () => void,
      reject: (e) => void,
      ...
      commands.forEach((command: IRawCommand) => {
        if (command.waitForResponse) this._commands.push(command);
      });
    }
  ...
  export class PerformanceMonitorBase extends MonitorBase {
    ...
    protected initialize() {
      ...
      this._powerCurveEvent.registerChangedEvent(
        this.enableDisableNotification.bind(this)
      );
    ...
    public sendCSafeBuffer(
      ...
      return new Promise((resolve, reject) => {
        ...
        if (this.sortCommands)
          rawCommandBuffer.sort((first, next) => {
    ...
    protected checkSendBuffer() {
      ...
      if (
        this._waitResonseBuffers.length == 0 &&
    ...
    protected sendBufferFromQueue(sendData: SendBufferQueued) {
      ...
      var waitBuffer = new WaitResponseBuffer(
        this,
        resolve,
        reject,
        sendData.rawCommandBuffer,
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        if (byteArray && byteArray.length > 0) {
          ...
          if (
            ...
            reject(
              `Csafe commands with length ${
                bytesToSend.length
              } does not fit into buffer with size ${this.getPacketSize()} `
            );
          else {
            ...
            while (sendBytesIndex < bytesToSend.length) {
              ...
              if (this.logLevel == LogLevel.trace)
                this.traceInfo(
                  "send csafe: " + utils.typedArrayToHexString(buffer, true)
                );

              this.driver_write(dataView)
                .then(() => {
    ...
    public handeReceivedDriverData(dataView: DataView) {
      ...
      if (
        ...
        if (this.logLevel == LogLevel.trace)
          this.traceInfo(
            "continious receive csafe: " +
              utils.typedArrayToHexString(dataView.buffer, true)
          );
        var i = 0;
        ...
        while (i < dataView.byteLength && !moveToNextBuffer) {
          ...
          if (!waitBuffer.stuffByteActive) {
            ...
            if (this.logLevel == LogLevel.trace)
              this.traceInfo(
                `parse: ${i}: ${utils.toHexString(currentByte, 1)} state: ${
                  waitBuffer.frameState
                } checksum:${utils.toHexString(waitBuffer.calcCheck, 1)} `
              );
            ...
            switch (waitBuffer.frameState) {
              ...
              case FrameState.statusByte: {
                ...
                waitBuffer.monitorStatus =
                  currentByte & csafe.defs.SLAVESTATE_MSK;
                waitBuffer.prevFrameState =
                  (currentByte & csafe.defs.PREVFRAMESTATUS_MSK) >> 4;
                if (this.logLevel == LogLevel.trace)
                  this.traceInfo(
                    `monitor status: ${waitBuffer.monitorStatus},prev frame state: ${waitBuffer.prevFrameState}`
                  );
              ...
              case FrameState.parseCommandLength: {
                ...
                if (
                  waitBuffer.statusByte == waitBuffer.command &&
                  currentByte == csafe.defs.FRAME_END_BYTE
                ) {
              ...
              case FrameState.parseCommandData: {
                if (!waitBuffer.commandData) {
                  ...
                  waitBuffer.commandData = new Uint8Array(
                ...
                if (waitBuffer.nextDataLength == 0) {
                  if (
                    waitBuffer.command < csafe.defs.CTRL_CMD_SHORT_MIN &&
        ...
        if (this._receivePartialBuffers) {
          ...
          if (moveToNextBuffer) waitBuffer = this.moveToNextBuffer();
          else if (
            dataView.byteLength != this.getPacketSize() &&
            waitBuffer &&
            waitBuffer.frameState != FrameState.initial
          ) {
