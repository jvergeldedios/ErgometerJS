...
namespace ergometer {
  ...
  export class PerformanceMonitorUsb extends PerformanceMonitorBase {
    ...
    static canUseUsb(): boolean {
      return (
        PerformanceMonitorUsb.canUseNodeHid() ||
        ...
        this.newCsafeBuffer()
          ...
          .then(() => {
            ...
            if (this.strokeState != previousStrokeState) {
              ...
              var doPowerCurveUpdate =
                this.strokeState == StrokeState.recoveryState;
              if (
                doPowerCurveUpdate ||
    ...
    protected autoUpdate(first = true) {
      ...
      if (
        this.connectionState == MonitorConnectionState.readyForCommunication &&
    ...
    protected isWaiting(): boolean {
      ...
      return (
        this.strokeState == StrokeState.waitingForWheelToAccelerateState &&
      ...
      return new Promise((resolve, reject) => {
        this.highResolutionUpdate()
          .then(() => {
            ...
            if (
              (this.trainingData.workoutState != WorkoutState.workoutRow &&
                diff > 200) ||
              (this.trainingData.workoutState == WorkoutState.workoutRow &&
                diff > 1000)
            ) {
            ...
            onDataReceived: (pace: number) => {
              ...
              if (pace > 0) {
                ...
                caloriesPerHour = Math.round(
                  (2.8 / (paced * paced * paced)) * (4.0 * 0.8604) + 300.0
                );
              }
          ...
          .getCadence({
            onDataReceived: (value: number) => {
              if (value > 0) {
                ...
                this.strokeData.strokesPerMinuteAverage =
                  this._nSPM / this._nSPMReads;
              }
            },
          })
          .getPower({
    ...
    protected trainingDataUpdate(): Promise<void> {
      ...
      return this.newCsafeBuffer()
        ...
        .then(() => {
          ...
          //}
          if (
            this.trainingData.workoutState == WorkoutState.workoutLogged &&
            ...
            if (
              this.trainingData.workoutType >=
                WorkoutType.fixedDistanceNoAplits &&
              this.trainingData.workoutType <= WorkoutType.fixedTimeSplits
            ) {
              if (
                this.trainingData.duration &&
                this.trainingData.duration > 0
              ) {
                //doing an fixed time
              ...
              } else if (this.trainingData.distance > 0) {
          ...
          if (
            this.trainingData.workoutState != WorkoutState.workoutLogged &&
            (this.trainingData.endDistance ||
              this.trainingData.endDistance != 0 ||
              this.trainingData.endDuration != 0 ||
              this.trainingData.endDuration)
          ) {
            this.trainingData.endDistance = 0;
            this.trainingData.endDuration = 0;
            changed = true;
